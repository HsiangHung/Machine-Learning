...
Given a precision-recall curve, plotting precision $P(r)$ `P(r)` as a function of recall $r$`r`, average precision computes the average value of $p(r)$ `p(r)` over the interval from $r=0$ `r=0` to $r=1$ `r=1` [[wiki: Mean average precision]][ Mean average precision]:
...
$$\textrm{MAP} = \frac{\sum^Q_{q=1}\textrm{Ave[P(q)]}}{Q},$$
