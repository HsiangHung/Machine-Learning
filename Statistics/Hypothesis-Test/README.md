...
For example, if $H_0$: $\mu=3$ but our sample has $\bar{X}=3.2, \textrm{SE} = 0.246$, we obtain the test statistic Z = (3.2-3)/0.246 = 0.81. For one trailed, p-value = 0.209.
...
* This is a pretty high probability, so we think that a sample mean of 3.2 or more is likely to happen simply by chance.
