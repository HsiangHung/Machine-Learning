...
NOTE don't confuse sample size $n$ with the number of samples. We randomly select $n$ observations from population, which is defined as sample size, and we repeat the sampling process multiple times, which is defined as the number of samples.
...
If population standard deviation $\sigma$ or proportion $p$ is unknown, use sample standard deviation $s$ and proportion $\hat{p}$ instead.

### Reference

* [Central Limit Theorem]: https://github.com/HsiangHung/Machine_Learning_Note/blob/master/Statistics/supplementary/Central_Limit_Theorem.pdf
[[The Role of Probability] Central Limit Theorem](https://github.com/HsiangHung/Machine_Learning_Note/blob/master/Statistics/supplementary/Central_Limit_Theorem.pdf)

* [Simple Learning Pro, The Central Limit Theorem (7.3)]: https://www.youtube.com/watch?v=ivd8wEHnMCg&t=322s
[[Simple Learning Pro] Simple Learning Pro, The Central Limit Theorem (7.3)](https://www.youtube.com/watch?v=ivd8wEHnMCg&t=322s)


## 2. Decision Error: Type I and Type II Error
...
where `Decision` means the statistical testing results; $H_0$ is the null hypothesis and $H_a$ is the alternate hypothesis.

* **Type I error**: Reject $H_0$ when $H_0$ is true. e.g. Declaring the defendant guilty when they are actually innocent.
* **Type II error**: Fail to reject $H_0$ when $H_a$ is true. e.g. Declaring the defendant innocent when they are actually guilty.
 
Here $\alpha$ is known as the level of statistical significance defining the probability of making type-I error, that given the null hypothesis is true you reject the hypothesis

$$\alpha = \textrm{Pr}\Big( \textrm{reject H}_0| \textrm{H}_0 \textrm{ is True} \Big).$$

Usually we want α < 0.05. But if Type 1 error is dangerous choosing $\alpha$
or especially costly, choose a small significance level (e.g. 0.01).

On the other hand, $\beta$ is the probability of making type-II error, that given the null hypothesis is false you fail to reject the hypothesis

$$\beta = \textrm{Pr}\Big( \textrm{fail to reject H}_0| \textrm{H}_0 \textrm{ is False} \Big). $$

Ideally, we want both lower $\alpha$, but also lower $\beta$. 

$1- \beta$ is the **power**: the probability of correctly rejecting $H_0$. Higher **power** has more reliable statistical testing. Usually we want power > 0.8.
...
* [Coursera: Inferential Statistics, Decision Errors]: https://www.coursera.org/learn/inferential-statistics-intro/lecture/XFpuP/decision-errors
[[Coursera: Inferential Statistics] Coursera: Inferential Statistics, Decision Errors](https://www.coursera.org/learn/inferential-statistics-intro/lecture/XFpuP/decision-errors)
...
$$s = \sqrt{\frac{(n_1-1)^2s^2_1+(n_2-1)^2s^2_2}{n_1+n_2-2}}.$$
...
The form for the effect size d resembles the computation for a t-test statistic, except for that t-test statistic includes a factor of $1/\sqrt{n}$,

$$t = \frac{\mu_1 -\mu_2}{SE} = \frac{\mu_1 -\mu_2}{\big(\frac{SD}{\sqrt{n}} \big)},$$
