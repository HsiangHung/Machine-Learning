...
$$L(\theta, \mathbf{x}) = - \sum^K_{c=1} \mathbb{I}(y=c)\log\big( h_{\theta}(\mathbb{x}) \big) = - \sum^K_{j=1} \mathbf{I}(y=c)\log \left( \frac{\exp (\theta^{(c)^T} \mathbb{x}) }{ \sum_c \exp (\theta^{(c)^T} \mathbf{x} ) } \right),$$
