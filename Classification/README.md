...
$$ p(y=c|x) = \textrm{Softmax}(z)_c = \frac{e^{(\theta_c)^T}\symbf{x}}{\sum_{c=1} e^{(\theta_c)^T}\symbf{x}}, $$


where $z_c$​ represents the $c$-th calss score (also known as logits) of the model. $p(y=c|x)$ is the probability of the input $x$ belonging to the class.
