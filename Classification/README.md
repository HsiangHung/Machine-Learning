...
$$L(\theta, \symbf{x}) = - \sum^K_{c=1}\symbf{I}(y=c)\log\big( h_{\theta}(\symbf{x}) \big) = - \sum^K_{c=1} \symbf{I}(y=c)\log \Big( \frac{e^{\theta^{(c)T}\symbf{x}}}{ \sum_{c=1} e^{\theta^{(c)T}\symbf{x}}} \Big)$$
