...
where $w_1$ and $w_2$ are in the same sentence. 

$\sqrt{d}$ is the normalization relevant to embedding dimensions. Here $\sqrt{d}=\sqrt{2}$. However, in the following, for simplicity, I just ignore it as $\sqrt{d}=1$. (This doesn't impact results of attentions.)
